...
static void fan_task(void *param) {
  ...
  while (1) {
    for (int i = 0; i < N_BOX; ++i) {
      ...
      int refOutput = 50;
      bool source_absent = is_ref_source_absent(get_box_fan_ref_source(i));
      if (source_absent) {
        refOutput = 100;
      } else if (rmin != rmax) {
      ...
      // Snap immediately when the sensor just dropped out: ventilating faster
      // is urgent, a slow ramp would defeat the point of the fallback.
      int v = source_absent ? target : step_towards(get_box_fan_duty(i), target, MAX_DUTY_STEP);
