...
typedef enum {
  MQTT_TIMING_DIAG_NONE = 0,
  MQTT_TIMING_DIAG_Q1 = 1,
  MQTT_TIMING_DIAG_Q2 = 2,
  MQTT_TIMING_DIAG_Q3 = 3,
  MQTT_TIMING_DIAG_Q4 = 4,
  MQTT_TIMING_DIAG_Q5 = 5,
} mqtt_timing_diag_mode_t;
...
static mqtt_timing_diag_mode_t mqtt_timing_diag_mode_from_client_id(const char *client_id) {
  if (client_id == NULL) {
    return MQTT_TIMING_DIAG_NONE;
  }
  ...
  if (strcmp(suffix, "-q1") == 0) {
    return MQTT_TIMING_DIAG_Q1;
  }
  if (strcmp(suffix, "-q2") == 0) {
    return MQTT_TIMING_DIAG_Q2;
  }
  if (strcmp(suffix, "-q3") == 0) {
    return MQTT_TIMING_DIAG_Q3;
  }
  if (strcmp(suffix, "-q4") == 0) {
    return MQTT_TIMING_DIAG_Q4;
  }
  if (strcmp(suffix, "-q5") == 0) {
    return MQTT_TIMING_DIAG_Q5;
  }
  return MQTT_TIMING_DIAG_NONE;
}

static bool mqtt_timing_diag_enable_discovery(mqtt_timing_diag_mode_t mode) {
  return mode >= MQTT_TIMING_DIAG_Q1;
}

static bool mqtt_timing_diag_enable_state_on_connect(mqtt_timing_diag_mode_t mode) {
  return mode >= MQTT_TIMING_DIAG_Q2;
}

static uint32_t mqtt_timing_diag_state_delay_ms(mqtt_timing_diag_mode_t mode) {
  switch (mode) {
    case MQTT_TIMING_DIAG_Q3:
      return 250;
    case MQTT_TIMING_DIAG_Q4:
      return 1000;
    case MQTT_TIMING_DIAG_Q5:
      return 3000;
    case MQTT_TIMING_DIAG_Q2:
    default:
      return 0;
  }
}
...
static void mqtt_task(void *param) {
  ...
  mqtt_timing_diag_mode_t timing_diag_mode = mqtt_timing_diag_mode_from_client_id(client_id);
  ESP_LOGI(SGO_LOG_NOSEND, "@MQTT Timing diag mode=%d", (int)timing_diag_mode);
  ...
  while(true) {
    if (xQueueReceive(cmd, &c, 10000 / portTICK_PERIOD_MS)) {
      if (c == CMD_MQTT_CONNECTED) {
        ...
        if (timing_diag_mode == MQTT_TIMING_DIAG_NONE) {
          if (first_connect) {
          ...
          if (first_connect && mqtt_timing_diag_enable_discovery(timing_diag_mode)) {
            ESP_LOGI(SGO_LOG_NOSEND, "@MQTT First connect");
            mqtt_publish_ha_discovery(client_id);
            first_connect = false;
          }
          if (mqtt_timing_diag_enable_state_on_connect(timing_diag_mode)) {
            uint32_t delay_ms = mqtt_timing_diag_state_delay_ms(timing_diag_mode);
            if (delay_ms > 0) {
