...
- **Heap watch**: a low-priority task samples the heap every 5 s; `/mqttdiag` and the MQTT diag topic report `heap_min_free_at` (uptime at which `heap_min_free` was last lowered) and `heap_low_events` (times free heap dipped under 8 KB), and a `@HEAP new minimum` warning is logged locally, so a transient heap dip can be correlated with what the app or the broker was doing at that moment.
