...
- `/mqttdiag` reports `mqtt_connected`, `ota_status`, `reset_reason` (esp_reset_reason_t of the current boot: 1 power-on, 3 software, 4 panic, 5/6/7 watchdogs, 9 brownout), `reset_history` (last 10 reset reasons, newest first), `heap_free`, `heap_min_free`, `uptime_s`, `nvs_used`/`nvs_free` (NVS entries on the 16 KB partition), `mqtt_stack_hwm` (bytes) and `time_valid` (clock past 2017); the same fields (minus `mqtt_disc_idx`/`wifi_status`/`reset_history`) are also published periodically over MQTT since the broker is a third-party service reached over the internet
